	...
	var inst = {
		...
		init: function(){
			...
			$('#fw-ext-backups-demo-list').on('click', '[data-install]', function(){
				...
				$( '#madara-dialog-confirm' ).css( 'dispplay', 'block' );

				if ( empty ) {
					$( "#madara-dialog-confirm" ).dialog({
					      resizable: false,
					      height: "auto",
					      width: 400,
					      modal: true,
					      open: function(){
						  	$('#madara-dialog-content').html( confirm_message );
						  },
					      buttons: {
					      	"Install Demo": function() {
					          $( this ).dialog( "close" );
					          $( '#madara-dialog-confirm' ).css( 'dispplay', 'none' );
					 ...
					 $( "#madara-dialog-confirm" ).dialog({
					      ...
					      buttons: {
					        ...
					        "Install Plugins": function() {
					          $( this ).dialog( "close" );
					          // $( '#madara-dialog-confirm' ).css( 'dispplay', 'none' );
					          var win = window.open( plugin, '_blank' );
					          win.focus();
					        },
					        Cancel: function() {
					          $( this ).dialog( "close" );
