jQuery(function($){
	$( document ).ajaxComplete(function( event, request, settings ) {
		if ( 'action=fw:ext:backups:status' == settings.data || 'action=fw%3Aext%3Abackups%3Astatus' == settings.data ) {
			...
			$.each( rows, function(i,e){
				...
				innerHTML += ' | <a href="#" data-package="'+mangaboothBackup.upload_base_url+'/fw-backup/'+downloadFile+'" class="madara-create-demo" id="create-'+id+'" data-confirm="Warning! You are about to create a demo content from this backup. Are you sure?">Create Demo</a>';
				$(e).html( innerHTML );
			} );
		}
	});
	...
	$(document).on( 'click', '.madara-create-demo', function(e){
		e.preventDefault();
